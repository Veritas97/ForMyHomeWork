...
## 14. Работа с удаленнами репозиториями

1. Для работы с удаленными репозиториями на требуется регистрация на `GitHub`
 * Важное примичание, может возникнуть ошибка при авторизации вашего аккаунта и вашего рабочего места - для этого на `GitHub` надо создать токен и авторизироватся.

2. Далее для того что бы скачать какий либо рипозиторий 
```
git clone "url-адреес репозитория"
   
   * Важно что такая команда может так же авторизовать локальный репозиторий если вы скачали его архивным файлом

```
3. Далее что бы взять удаленный репозиторий. Нам требуется следующия команда, то есть выгрузить наш репозиторий в общий доступ.

```
git remote add origin "url-адреес репозитория"
```
Потом нам требуется обьяснить `GitHub`-у обьяснить где оснавная ветка
Используем команду 
```
git branch -M main
```
Ну и в конце нам надо выгрузить репозиторий на `GitHub`
```
git push -u origin main
```

4. Для того что бы вытянуть из нашего или другого файла его изменения
```
git pull
```
5. Что бы выгрузить все наши изменения используем команду 
``` 
git push
```
## 15. Работа с удаленными репозиториями

Для того что бы скачать удаленный репозиторий на требуется след команда:
```
git clone "url-ссылка"
```

далее как сказали на надо перейти в ту папку куда скачался реопзиторий
```
cd "наименование папки"
```
Ну и теперь мы можем работать в том что скачали

## 16. Работа по OpenSource
Мы можем работать с открытами кодами и для этого нам для начала нам надо добавить репозиторий к себе на аккаунт.

Для этого заходим на `GitHub` и ищем нас интересующий проект.

Как нашли нужный нам проект. Добавляем данный репозиторий к себе на аккаунт в `GitHub`

![FORK](Fork.png)
Добавляем его через данную функцию сайта. Ну и потом просто скачиваем к себе.

* Важно в не нашем репозитории требуется создать отдельную ветку что бы не ломать основной файл.

Ну выгружаем через туже команду:
```
git push - важно гид нам скажет полную команду для отправки 
```
После того как мы выгрузили, на оригинальной ветке появится кнопка 

![Кнопка](Кнопка.png)

Данная кнопка просит подвердить и сделать запрос на внедрения от основного чувака.
